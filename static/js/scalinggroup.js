  ...
  $.widget('eucalyptus.scalinggroup', $.eucalyptus.eucawidget, {
    ...
    _create : function() { 
      ...
      $snapSelector.change( function(){
        ...
        if (snapshotId) {
          var snapshot = eucadescribe('snapshot', snapshotId);
    ...
    _initAttachDialog : function(dfd) {  // should resolve dfd object
      ...
      if(!$volumeSelector.val()){ // launch from instance landing
        ...
        var results = eucadescribe('instance');
        var instance = null;
        if (results){
          for( res in results){
